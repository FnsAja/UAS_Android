...
$performance = trim($_POST["performance"]); //menghilangkan spasi di performance

if ((empty($fullname)) || (empty($username)) || (empty($password)) || (empty($address)) || (empty($email)) || (empty($phone)) || (empty($division)) || (empty($performance))) {
	$response = new user();
	$response->success = 0;
			...
			if(preg_match($username_pattern, $username)){
				if(preg_match($email_pattern, $email)){
					if(preg_match($password_pattern, $password)){
						$query = mysqli_query($con, "INSERT INTO users (nama, username, password, alamat, email, notelp, divisi, performance) VALUES ('".$fullname."','".$username."', '".$password."', '".$address."', '".$email."', '".$phone."', '".$division."', '".$performance."')");
				...
				$response = new user();
				$response->success = 0;
				$response->message = "Mohon untuk tidak menggunakan simbol pada username.";
				die(json_encode($response));
			}
		}
	}
